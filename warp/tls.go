...
import (
	...
	"github.com/Mostbesep/EscapeRoute/ip"
...
func (d *Dialer) TLSDial(plainDialer *net.Dialer, network, addr string) (net.Conn, error) {
	...
	newIp, err := ip.RandomIPFromPrefix(netip.MustParsePrefix("141.101.113.0/24"))
	if err != nil {
		return nil, err
	}
	plainConn, err := plainDialer.Dial(network, newIp.String()+":443")
	if err != nil {
		return nil, err
	}
